# Compensation Audit Tools

## Overview

The `tools/` directory contains all backend logic for the Compensation Structure Audit Tool. This includes data loading and constants, multi-source web scrapers, validated market analysis, and a neural network classifier for semantic role-to-level mapping. Each module is designed to be imported independently and used from either the Streamlit UI, the CLI, or the Python API.
...
```txt
tools/
    auditUtils.py              Core constants, salary bands, data loaders, theme
    syntheticData.py           Synthetic, fictional salary framework constants
    careerPageScraper.py       Platform-specific career page scrapers
    compensationScraper.py     Multi-source web scraper (11 data sources)
    referenceParser.py         Markdown reference document parser
    validatedAnalysis.py       Backend for validated market analysis (8 features)
    featureEngineering.py      Feature extraction for neural network classifier
    neuralClassifier.py        MLP-based level classifier (scikit-learn)
    nnVisualizations.py        9 Plotly visualization functions for NN
    .modelCache/               Trained model artifacts (gitignored)
    .scraperCache/             Cached HTTP responses (gitignored)
    .validatedCache/           Validated analysis snapshots (gitignored)
```
...
| Module | Lines | Purpose | Detailed Docs |
|--------|-------|---------|---------------|
| `auditUtils.py` | ~1000 | Salary bands, level definitions, calculation functions, theme, data loaders | `documentation/auditUtilsREADME.md` |
| `syntheticData.py` | ~120 | Synthetic, fictional salary framework constants imported by `auditUtils.py` | -- |
| `compensationScraper.py` | ~2800 | Multi-source compensation scraper (BLS, FRED, H1B, Glassdoor, etc.) | `documentation/scraperREADME.md` |
| `careerPageScraper.py` | ~1100 | Career page scrapers (Greenhouse, Lever, Workday, generic HTML, OCR) | `documentation/scraperREADME.md` |
| `referenceParser.py` | ~600 | Parses markdown reference docs for company lists, SOC codes, career page configs | -- |
| `validatedAnalysis.py` | ~1500 | Geographic normalization, role mapping, band recalculation, trend analytics, NN integration | -- |
| `featureEngineering.py` | ~470 | Converts job listings into 60-dimensional numeric feature vectors | `documentation/neuralClassifierREADME.md` |
| `neuralClassifier.py` | ~690 | `BucketClassifier` class wrapping scikit-learn `MLPClassifier` | `documentation/neuralClassifierREADME.md` |
| `nnVisualizations.py` | ~710 | 9 Plotly visualization functions for model inspection and education | `documentation/neuralClassifierREADME.md` |
...
```txt
Career Pages (26 companies)           Synthetic Survey CSV
    |                                        |
    v                                        v
CareerPageScraper                    loadMarketSurveyData()
    |                                        |
    v                                        v
Normalize to surveyColumns           Training data (Vlvl labels)
    |                                        |
    v                                        v
filterEngineeringRoles()             buildFeatureMatrix()
    |                                        |
    v                                        v
applyGeoNormalization()              BucketClassifier.train()
    |                                        |
    v                                        v
assignBuckets()  <------- NN signal ---- .modelCache/
    |
    |   Combines 4 signals:
    |     1. YoE from description (regex)
    |     2. Title keywords (heuristic)
    |     3. Salary band proximity
    |     4. Neural network prediction
    |
    v
LevelBucket (Level 1-5), BucketConfidence, ManagementFlag
    |
    v
Streamlit UI / Excel Export
```
...
- **No nonlinear feature combinations.** A 'Senior Engineer' at a seed-stage startup paying $95K and a 'Senior Engineer' at SpaceX paying $165K both match the 'senior' keyword, but they belong in different levels. The keyword system cannot distinguish them without separate salary logic that operates independently.
...
The neural network addresses these by learning from the labeled job listings in the synthetic market survey (`documentation/marketSurveySynthetic.csv`). It discovers feature interactions that the keyword system cannot express. The bundled synthetic survey is small, so the trained classifier is a demonstration of the pipeline rather than a production-accurate model — the figures in the sections below describe a training run on a larger labeled dataset.
...
- Load the survey dataset via `loadMarketSurveyData()`
- Extract feature matrix (60 features) and label vector (`Vlvl` column, levels 1-5)
- Filter out any unlabeled samples (class 0) before training
...
Three cache directories live inside `tools/`. All are gitignored.

| Directory | Purpose | Contents |
|-----------|---------|----------|
| `.scraperCache/` | HTTP response caching for web scrapers | JSON files keyed by URL hash |
| `.validatedCache/` | Timestamped snapshots of validated analysis runs | CSV data files + JSON metadata |
| `.modelCache/` | Trained neural network artifacts | `trained_model.joblib`, `scaler.joblib`, `metadata.json` |

Deleting any cache directory is safe -- it will be recreated on next use. Deleting `.modelCache/` disables the NN signal and falls back to keyword-only classification.
...
| `numpy` | >= 1.22 | All modules |
| `plotly` | >= 5.0 | `nnVisualizations.py`, all Streamlit views |
| `streamlit` | >= 1.30 | All views |
| `scikit-learn` | >= 1.4 | `neuralClassifier.py`, `nnVisualizations.py` |
| `joblib` | >= 1.2 | `neuralClassifier.py` (model serialization) |
| `openpyxl` | >= 3.0 | `auditUtils.py` (Excel file reading) |
| `requests` | >= 2.28 | `compensationScraper.py` (HTTP requests) |
| `beautifulsoup4` | >= 4.12 | `compensationScraper.py` (HTML parsing) |

Install the NN-specific dependency:
